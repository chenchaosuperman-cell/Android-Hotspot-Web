...
- **新增「新设备接入通知」**：首次出现的客户端 MAC 自动记录并推送（PushPlus/钉钉/Bark/Server酱），同一设备只提醒一次；配置键 `NOTIFY_NEW_DEVICE`。
- **新增「每日定时重启热点」**：到点自动重启热点，缓解长时间运行卡顿/断流；配置键 `RESTART_DAILY_ENABLE` / `RESTART_DAILY_TIME`（默认 03:00，当日只执行一次）。
- **新增「流量耗尽预测」**：首页套餐栏按当前账期日均消耗估算剩余流量可用天数（`predictDays`）。
- **新增通知渠道：Bark（iPhone 推送）与 Server酱（微信推送）**：通知队列升级为 8 字段（pp/dt/bk/sc 渠道独立状态 + 失败退避重试），旧队列自动迁移；支持单渠道测试与健康状态展示。
- **新增「登录失败锁定」**：管理页密码连续错误 5 次锁定 5 分钟（防暴力破解），前端展示剩余次数与锁定倒计时。
- **新增「设备适配检测」**：诊断面板显示 Root 框架（KernelSU/Magisk/APatch）、Android 版本、SDK、busybox/ip/iptables/softap/su 可用性，便于跨机型排查。
- **新增自动化回归测试骨架**：`tests/run_tests.sh` 对 MAC/时间/base64url/厂商识别等核心函数做断言（本机与真机均可运行）。
